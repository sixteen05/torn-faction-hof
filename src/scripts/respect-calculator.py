...
def calculate_respect_totals_v3(attacks: List[Dict]) -> Dict[str, float]:
    ...
    for atk in attacks:
        attacker_name = (
            atk["attacker"]["name"]
            if atk["attacker"]["name"] != ""
            else atk["attacker"]["id"]
        )
        hits_count[attacker_name] += 1
        if atk["chain"] in BONUS_HITS:
            continue

        mods_avg[attacker_name] += float(atk["modifiers"].get("fair_fight", 1))
        totals[attacker_name] += float(atk["respect_gain"])
        avg_count[attacker_name] += 1

    for attacker in totals:
        if avg_count[attacker] > 0:
            avg[attacker] = totals[attacker] / avg_count[attacker]
            mods_avg[attacker] /= avg_count[attacker]

    return dict(totals), dict(avg), dict(hits_count), dict(mods_avg)
